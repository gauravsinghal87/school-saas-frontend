    ...
    return (
        <div className="relative w-full sm:w-64">
            ...
            <input
                ...
                className="w-full h-9 pl-9 pr-4 text-sm rounded-xl border border--border bg-surface-card text--primary placeholder:text-secondary outline-none transition-all focus:border-primary focus:ring-2 focus:ring-primary/20"
            />
        </div>
    );
}
    ...
    return (
        <div className="flex items-center gap-2 text-sm text-secondary">
            ...
            <select
                ...
                className="h-9 px-2 pr-7 rounded-xl text-sm border border--border bg-surface-card text-primary outline-none cursor-pointer focus:border-primary focus:ring-2 focus:ring-primary"
            >
...
function Pagination({ page, totalPages, total, pageSize, onPage }) {
    ...
    const ghost = `${base} text-secondary hover:bg-surface-page hover:text--primary`;
    const active = `${base} bg-primary text-white shadow-sm`;
    const inactive = `${base} text-secondary hover:bg-surface-page`;

    return (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-1">
            <p className="text-xs text-secondary order-2 sm:order-1">
                {total === 0 ? "No results" : `Showing ${from}–${to} of ${total} records`}
            </p>
            <div className="flex items-center gap-1 order-1 sm:order-2">
                <button className={ghost} onClick={() => onPage(1)} disabled={page === 1} title="First">
                    <Icon.ChevronsLeft />
                </button>
                <button className={ghost} onClick={() => onPage(page - 1)} disabled={page === 1} title="Previous">
                    <Icon.ChevronLeft />
                </button>
                <div className="flex items-center gap-1 mx-1">
                    {pageWindow[0] > 1 && (
                        <>
                            ...
                            {pageWindow[0] > 2 && <span className="w-8 text-center text-xs text-secondary">…</span>}
                        </>
                    )}
                    {pageWindow.map((p) => (
                    ...
                    {pageWindow[pageWindow.length - 1] < totalPages && (
                        <>
                            {pageWindow[pageWindow.length - 1] < totalPages - 1 && (
                                <span className="w-8 text-center text-xs text-secondary">…</span>
                            )}
                            <button className={inactive} onClick={() => onPage(totalPages)}>{totalPages}</button>
                        </>
                    )}
                </div>
                <button className={ghost} onClick={() => onPage(page + 1)} disabled={page === totalPages || totalPages === 0} title="Next">
                    <Icon.ChevronRight />
                </button>
                <button className={ghost} onClick={() => onPage(totalPages)} disabled={page === totalPages || totalPages === 0} title="Last">
                    <Icon.ChevronsRight />
                </button>
            </div>
        </div>
...
function ActionCell({ row, onEdit, onDelete }) {
    return (
        <div className="flex items-center gap-1">
            {onEdit && (
                <button onClick={() => onEdit(row)} title="Edit" className="inline-flex items-center justify-center w-8 h-8 rounded-lg text-secondary hover:text-primary hover:bg-primary transition-all duration-150">
                    <Icon.Edit />
                </button>
            )}
            {onDelete && (
                <button onClick={() => onDelete(row)} title="Delete" className="inline-flex items-center justify-center w-8 h-8 rounded-lg text-secondary hover:text-error hover:bg-error transition-all duration-150">
                    <Icon.Trash />
                </button>
            )}
        </div>
...
// DataTable
//
// CLIENT mode  (default):  omit serverMode — table filters/sorts/paginates internally
//
// SERVER mode:             pass serverMode + these props from parent:
//   page, total, pageSize, onSearch, onPageChange, onPageSizeChange
// ─────────────────────────────────────────────────────────────
export default function DataTable({
    ...
    const activePage = isServer ? (externalPage ?? 1) : internalPage;
    const activePageSize = isServer ? (externalTotal != null ? data.length : internalPageSize) : internalPageSize;
    const activeTotal = isServer ? (externalTotal ?? 0) : sorted.length;
    const activeTotalPages = isServer
        ? Math.max(1, Math.ceil(activeTotal / (internalPageSize || defaultPageSize)))
        : Math.max(1, Math.ceil(sorted.length / internalPageSize));

    const displayRows = isServer
        ? data
        : sorted.slice((Math.min(internalPage, activeTotalPages) - 1) * internalPageSize, Math.min(internalPage, activeTotalPages) * internalPageSize);
    ...
    return (
        <div className="flex flex-col rounded-2xl border border--border bg-surface-card overflow-hidden shadow-sm">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-5 py-4 border-b border--border">
                ...
                <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                    ...
                    {onAdd && (
                        <button
                            onClick={onAdd}
                            className="inline-flex items-center gap-1.5 h-9 px-4 rounded-xl text-sm font-semibold bg-primary text-button-primary-text hover:bg-button-primary-hover transition-all duration-150 shadow-sm hover:shadow active:scale-[0.97] whitespace-nowrap"
                        >
                            <Icon.Plus />
                            {addLabel}
                        </button>
                        ...
                        <tr className="bg-surface-page">
                            {allCols.map((col) => (
                                <th
                                    ...
                                    className={`group px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-secondary select-none whitespace-nowrap border-b border--border ${col.sortable ? "cursor-pointer hover:text--primary transition-colors" : ""}`}
                                >
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            displayRows.map((row, idx) => (
                                <tr
                                    key={row[rowKey] ?? idx}
                                    className="border-b border--border last:border-0 hover:bg-surface-page/70 transition-colors duration-100"
                                >
                                    {allCols.map((col) => (
                                        <td key={col.key} className="px-4 py-3.5 text--primary whitespace-nowrap">
                                            {col.key === "__actions__" ? (
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-5 py-4 border-t border--border">
                <PageSizeSelect
                    value={isServer ? (externalTotal != null ? defaultPageSize : internalPageSize) : internalPageSize}
                ...
                <Pagination
                    ...
                    total={activeTotal}
                    pageSize={isServer ? defaultPageSize : internalPageSize}
